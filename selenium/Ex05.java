...
public class Ex05 {
	public static void main(String[] args) {
		...
		for(int i=1;i<=18;i++) {
			...
			// WebElement area = map.findElement(By.xpath("//*[@key=\"map" + String.format("%02d", i) + "\"]"));
	...
	private static void mapShow(WebDriver driver) {
		System.out.println("-".repeat(100));
		List<WebElement> areas = driver
				                 .findElements(By.cssSelector("#content>div>p"))
				                 .get(1)
				                 .findElements(By.cssSelector("map area"));
		System.out.println("영역 : " + areas.size() + "개");
		for(WebElement e : areas) {
			if(!e.getAttribute("alt").equals("undefined")) {
				System.out.print(e.getAttribute("alt") + " : ");
