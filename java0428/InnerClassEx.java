...
/*
 내부 클래스 : 클래스 내부에 클래스를 만든다. 책은 9장!!
            묶어 놓는것이 더 효과적일때, 외부 클래스의 변수에 접근하기 위해서
 인스턴스 내부 클래스
 정적 내부 클래스 : static이 붙은 내부 클래스
 로컬 내부클래스 : 메서드 안에서 만든 클래스. 메서드 안에서만 사용
*/
...
class Outer{
	...
	static class StaticInner{
		...
		public void show() {
			...
			class LocalClass{
				public void show() {
					System.out.println(x); //1.7이전에는 final인 지역변수만 접근가능
				}
			}
			LocalClass localClass = new LocalClass();
			
			//System.out.println(outer); //static 클래스에서는 static멤버만 사용가능 
...
public class InnerClassEx {
	...
	public static void main(String[] args) {
		...
		// 익명 내부 클래스 : 다른곳에서는 사용하지않고 이곳에서 딱1번만 사용할 경우에 사용 
		new Show() {	
			@Override
